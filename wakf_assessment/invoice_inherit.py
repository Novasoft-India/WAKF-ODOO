...
class invoice_inherit(osv.osv):
    ...
    #         'report_name': 'account_print_invoice',
    #         'datas': datas,
    #         }
    #===========================================================================
        ...
        if view_type == 'tree':
            ...
            for node in doc.xpath("//field[@name='partner_id']"):
                ...
                if partner_string == 'Applicant Name':
                    ...
                    for node in doc.xpath("//field[@name='partner_id']"):
                        node.set('string', 'Applicant Name')
...
class assessment_window(osv.osv):
    ...
    def on_change_wakf_regno_to_name_new_assess(self, cr, uid, ids, reg_no, acc_year, context=None):
        ...
        if reg_no:
            ...
            if similar_objs:
                ...
                taluk = id_res_partner.browse(cr, uid, search_ids, context=context)[0].wakf_taluk_id.id
                village = id_res_partner.browse(cr, uid, search_ids, context=context)[0].village_id.id
                values = {
                            'wakf_id': output,
                            'district':district,
                            'taluk':taluk,
    ...
    _columns = {
                ...
                'taluk':fields.many2one('wakf.taluk','Taluk',ondelete='set null'),
                'village':fields.many2one('wakf.village','Village',ondelete='set null'),
