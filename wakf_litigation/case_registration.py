...
class case_registration(osv.osv):
    ...
    def create(self, cr, uid, data, context=None):
        ...
        if type:
            if type == 'aa':
                data['case_no']=self.pool.get('ir.sequence').get(cr, uid, 'CASEAA')
            if type == 'ep':
                data['case_no']=self.pool.get('ir.sequence').get(cr, uid, 'CASEEP')
            if type == 'op':
                data['case_no']=self.pool.get('ir.sequence').get(cr, uid, 'CASEOP')
            if type == 'coveat':
                data['case_no']=self.pool.get('ir.sequence').get(cr, uid, 'CASECO')
    ...
    def action_dismiss(self, cr, uid, ids, context=None):
        ...
        return False
    def action_proceed(self, cr, uid, ids, context=None):
        self.write(cr,uid,ids,{'state':'proceeding'})
        return False
    ...
    def on_change_wakf_regno_to_name(self, cr, uid, ids, reg_no, context=None):
        ...
        if reg_no:
            ...
            if search_ids:
                ...
                values={'wakf_name':similar_objs.id,
                        ...
                        'wakf_taluk':similar_objs.wakf_taluk_id.id,
                        'wakf_village':similar_objs.village_id.id,
                        'wakf_address':similar_objs.comm_addr or False,
                        }
            else:
        ...
        return {'value' :values}
    def on_change_account_to_type(self, cr, uid, ids, reg_no,account_year, context=None):
        if reg_no and account_year:
            values = {}
            search_condition = [('assess_year_saleorder','=',account_year),('registration_no','=',reg_no),'|',('state','=','open'),('state','=','draft')]
            search_ids = self.pool.get('account.invoice').search(cr,uid,search_condition)
            if search_ids:
                browse_ids = self.pool.get('account.invoice').browse(cr,uid,search_ids)[0]
                type_assess = browse_ids.assessment_type
                if type_assess:
                    values={'assess_type' : type_assess}
            else:
                values={'assess_type' : False}
        return {'value' :values}
    
    _columns = {
        'case_no': fields.char('Case No',readonly=True),
        ...
        'assess_type': fields.char('Assessment Type'),   #  For AA
        'case_year': fields.date('Date of Filing'), 
        ...
        'wakf_reg_no': fields.integer('Wakf register No',required=True),
        'wakf_name': fields.many2one('res.partner','Wakf Name',domain=[('customer','=',True)],ondelete="set null"),
        ...
        'board_meeting_date': fields.datetime('Board Meeting Date',required=True),
