...
class sws_category_education(osv.osv):
    ...
    _columns = {
            'course_name':fields.many2one('sws.scholar.criteria.course','Course Name',domain=[('allowed','=',True)]),
            'qualification':fields.many2one('sws.scholar.criteria.qualification','Qualification',domain=[('allowed','=',True)]),
            'qualifying_mark':fields.integer('Qualifying Mark :', required=False),
            'out_of':fields.integer('Out Of', required=False),
            'qualifying_percentage':fields.float('Qualifying % :',size=64, required=False),
            'date_of_birth':fields.date('Date of Birth:', required=False),
            'place_of_birth':fields.char('Place of Birth:', required=False),
            'married':fields.boolean('Married', required=False),
            'sex': fields.selection([
                ('male', 'Male'),
                ('female', 'Female'),
                ], 'Sex'),
            'sureitie1_name':fields.char('Sureitie Name (1)',size=256, required=False),
            'sureitie2_name':fields.char('Sureitie Name (2)',size=256, required=False),
            'age':fields.integer('Age:', size=8, readonly=True),
            'total_annual_income':fields.float('Total Annual Income:', required=False),
            'institution_name':fields.many2one('sws.scholar.criteria.institution','Institution Name:',required=False),
            'sws_id':fields.many2one('sws.registration','PENSION',ondelete='set null'),
...
class pension_cycle(osv.osv):
    ...
    def fetch_pending_pensions(self, cr, uid, ids, context=None):
        pending_list = []
        del_a = []
        date_reference = False
        for_month = False
        ...
        for rec in self.browse(cr,uid,ids,context):
            ...
            for records in res_partner_browse:
                ...
                for line in records.history_transaction:
                    if line.status == 'pending':
                        if line.date_sanction <= date_limit:
                            amount = line.amount
                            year = line.year
                            for_month = line.for_month
                            date_reference = line.date_sanction
                            ...
                            pending_list.append((0,0,{'category':category,'head':head,'application_no':application_no,'date_reference':date_reference,'name_appli':name_appli,'date_year':year,'date_month':for_month,'sanctioned_amount':amount,'status':status,'full_reconcile':False}))
        for rec in self.browse(cr,uid,ids,context):
            for records in rec.pension_line_id:
                del_a.append((2, records.id))
        self.write(cr, uid, ids, {'pension_line_id': del_a}, context=context)
        self.write(cr,uid,ids,{'pension_line_id':pending_list})
        return False
